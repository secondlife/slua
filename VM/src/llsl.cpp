...
static int lsl_detected_event_index(lua_State *L)
{
...
static int lsl_detected_event_call_ll_function(lua_State *L, const char *function_name)
{
    const lua_DetectedEvent *detected_event = (const lua_DetectedEvent *)lua_touserdatatagged(L, 1, UTAG_DETECTED_EVENT);
    if (!detected_event)
        luaL_typeerror(L, 1, "DetectedEvent");
...
static int lsl_detected_event_get_owner(lua_State *L)
{
    return lsl_detected_event_call_ll_function(L, "DetectedOwner");
}

static int lsl_detected_event_get_name(lua_State *L)
{
    return lsl_detected_event_call_ll_function(L, "DetectedName");
}

static int lsl_detected_event_get_key(lua_State *L)
{
    return lsl_detected_event_call_ll_function(L, "DetectedKey");
}

static int lsl_detected_event_get_face(lua_State *L)
{
    return lsl_detected_event_call_ll_function(L, "DetectedFace");
}

static int lsl_detected_event_get_link_number(lua_State *L)
{
    return lsl_detected_event_call_ll_function(L, "DetectedLinkNumber");
}

static int lsl_detected_event_adjust_damage(lua_State *L)
{
    const lua_DetectedEvent *detected_event = (const lua_DetectedEvent *)lua_touserdatatagged(L, 1, UTAG_DETECTED_EVENT);
    if (!detected_event)
        luaL_typeerror(L, 1, "DetectedEvent");
...
static int lsl_detected_event_tostring(lua_State *L)
{
    const lua_DetectedEvent *detected_event = (const lua_DetectedEvent *)lua_touserdatatagged(L, 1, UTAG_DETECTED_EVENT);
    if (!detected_event)
        luaL_typeerror(L, 1, "DetectedEvent");
...
static int lsl_llevents_index(lua_State *L)
{
...
static int lsl_llevents_newindex(lua_State *L)
{
    const lua_LLEvents *llevents = (const lua_LLEvents *)lua_touserdatatagged(L, 1, UTAG_LLEVENTS);
    if (!llevents)
        luaL_typeerror(L, 1, "LLEvents");
...
static int lsl_llevents_on(lua_State *L)
{
    const lua_LLEvents *llevents = (const lua_LLEvents *)lua_touserdatatagged(L, 1, UTAG_LLEVENTS);
    if (!llevents)
        luaL_typeerror(L, 1, "LLEvents");
...
static int lsl_llevents_off(lua_State *L)
{
    auto *sl_state = LUAU_GET_SL_VM_STATE(lua_mainthread(L));
...
static int lsl_llevents_once_wrapper(lua_State *L)
{
...
static int lsl_llevents_once_wrapper_cont(lua_State *L, int status)
{
...
static int lsl_llevents_once(lua_State *L)
{
    const lua_LLEvents *llevents = (const lua_LLEvents *)lua_touserdatatagged(L, 1, UTAG_LLEVENTS);
    if (!llevents)
        luaL_typeerror(L, 1, "LLEvents");
    ...
    lua_pushvalue(L, 2); // event_name
    lua_pushvalue(L, 3); // handler
    lua_pushcclosurek(L, lsl_llevents_once_wrapper, "once_wrapper", 3, lsl_llevents_once_wrapper_cont);
...
static int lsl_llevents_listeners(lua_State *L)
{
    const lua_LLEvents *llevents = (const lua_LLEvents *)lua_touserdatatagged(L, 1, UTAG_LLEVENTS);
    if (!llevents)
        luaL_typeerror(L, 1, "LLEvents");
...
static int lsl_llevents_eventnames(lua_State *L)
{
    const lua_LLEvents *llevents = (const lua_LLEvents *)lua_touserdatatagged(L, 1, UTAG_LLEVENTS);
    if (!llevents)
        luaL_typeerror(L, 1, "LLEvents");
...
int lsl_llevents_handle_event_cont(lua_State *L, int status)
{
...
static int lsl_llevents_handle_event_init(lua_State *L)
{
    ...
    return lsl_llevents_handle_event_cont(L, LUA_OK);
}
...
int luaopen_sl(lua_State* L)
{
    ...
    lua_pushcfunction(L, lsl_detected_event_index, "__index");
    lua_setfield(L, -2, "__index");

    lua_pushcfunction(L, lsl_detected_event_tostring, "__tostring");
    lua_setfield(L, -2, "__tostring");

    // Add the methods to the metatable
    lua_pushcfunction(L, lsl_detected_event_get_owner, "getOwner");
    lua_setfield(L, -2, "getOwner");

    lua_pushcfunction(L, lsl_detected_event_get_name, "getName");
    lua_setfield(L, -2, "getName");

    lua_pushcfunction(L, lsl_detected_event_get_key, "getKey");
    lua_setfield(L, -2, "getKey");

    lua_pushcfunction(L, lsl_detected_event_get_face, "getFace");
    lua_setfield(L, -2, "getFace");

    lua_pushcfunction(L, lsl_detected_event_get_link_number, "getLinkNumber");
    lua_setfield(L, -2, "getLinkNumber");

    lua_pushcfunction(L, lsl_detected_event_adjust_damage, "adjustDamage");
    ...
    lua_setuserdatadtor(L, UTAG_LLEVENTS, lsl_llevents_dtor);
    ...
    lua_pushcfunction(L, lsl_llevents_index, "__index");
    lua_setfield(L, -2, "__index");

    lua_pushcfunction(L, lsl_llevents_newindex, "__newindex");
    lua_setfield(L, -2, "__newindex");

    // Add the event management methods to the metatable
    lua_pushcfunction(L, lsl_llevents_on, "on");
    lua_setfield(L, -2, "on");

    lua_pushcfunction(L, lsl_llevents_off, "off");
    lua_setfield(L, -2, "off");

    lua_pushcfunction(L, lsl_llevents_once, "once");
    lua_setfield(L, -2, "once");

    lua_pushcfunction(L, lsl_llevents_listeners, "listeners");
    lua_setfield(L, -2, "listeners");

    lua_pushcfunction(L, lsl_llevents_eventnames, "eventNames");
    lua_setfield(L, -2, "eventNames");

    lua_pushcclosurek(L, lsl_llevents_handle_event_init, "handleEvent", 0, lsl_llevents_handle_event_cont);
