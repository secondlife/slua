...
std::optional<BcFunction> fromFunctionBytecode(std::string bytecode, std::vector<std::string_view>& strings)
{
    ...
    // ServerLua: We need to copy this to a 4-byte aligned memory address to appease UBSan.
    std::vector<Instruction> code(codesize);
    memcpy(code.data(), data + offset, size_t(codesize) * sizeof(Instruction));
    ...
    if (!buildFunctionGraph(fn, code.data(), codesize, lines, insnsPC))
        return {};
